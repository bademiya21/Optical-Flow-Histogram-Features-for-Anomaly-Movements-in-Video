...
void ActivityDetection::Inititalization()
{
	...
	// tweak parameters
		WinSize				= 32;
		WinSize2			= 15;
		ObsWindow			= 120;	//Original: 300; SG:120
		ProbWindow			= 30;
...
void ActivityDetection::OptFlowLK(Mat oldFrame,Mat newFrame, int index, string CurrFrameName)
{
	...
	calcOpticalFlowFarneback(PreMat, CurrMat, FlowField, 0.5, 5, 5, 2, 5, 1.1, OPTFLOW_FARNEBACK_GAUSSIAN);
...
void ActivityDetection::OptFlowEnergy_Frame()
{
	...
	float energy = sum(mEnergy_Frame).val[0];  
	ObservationBuf.push_back(energy);
	ThresholdBuf.push_back(energy);   
	OptFlowEnergy_Window(mEnergy_Frame);
	//Ee Sin 26 Nov
	mEnergy_Frame.release();
}
//<<--
//25 Oct 2013 SG: Replace the code -->>
/*void ActivityDetection::OptFlowEnergy_Frame()
{
	Mat Vx_sum,Vy_sum;
	Mat Energy_Frame;
    ...
    int nframe;
	Size msize;
	msize.height = VelYBuff.at(0).rows;
	msize.width  = VelXBuff.at(0).cols;
   
	tmp_Buff = Mat(msize,CV_32FC1); 
	Vx_sum   = Mat::zeros(msize,CV_32FC1);
	Vy_sum   = Mat::zeros(msize,CV_32FC1);
	Energy_Frame =  Mat::zeros(msize,CV_32FC1);
	// linear combination of matrix 
	nframe = (int)VelYBuff.size();
	for(int i=0; i < nframe; i++)
	{
		
		Vx_sum = Vx_sum + VelXBuff.at(i);
		
		Vy_sum = Vy_sum + VelYBuff.at(i) ;
	}
	
	Vx_sum = Vx_sum/WinSize2;
	Vy_sum = Vy_sum/WinSize2;
	

	cv::pow(Vx_sum,2.0,Vx_sum);
	cv::pow(Vy_sum,2.0,Vy_sum);
	
	
	Vx_sum = Vx_sum + Vy_sum;
	

	
	cv::sqrt(Vx_sum,Energy_Frame);

	
	// Added for debugging purpose
	//TmpMag = TmpMag.reshape(0,Energy_Frame.rows*Energy_Frame.cols);
	//Mat SortOut;
	//cv::sortIdx(TmpMag,SortOut,CV_SORT_EVERY_COLUMN+CV_SORT_DESCENDING);
	//Mat TopVals;
	//TopVals = SortOut.rowRange(0,4000);


	//double minVal; double maxVal; Point minLoc; Point maxLoc;
	//minMaxLoc(Energy_Frame,&minVal,&maxVal,&minLoc,&maxLoc,Mat());
	//FILE *tmpFile_det2;
	//tmpFile_det2 = fopen("..\\Output\\minmaxflow.txt","a+");
	//fprintf(tmpFile_det2,"%.05f,    %.05f\n",minVal,maxVal);
	//fclose(tmpFile_det2);



	
	// Energy of the certain number of frames
	Scalar scalEng;
	scalEng = sum(Energy_Frame);
	energy = sum(Energy_Frame).val[0];

	//Energy_Buff.push_back(energy);  
	ObservationBuf.push_back(energy);
	ThresholdBuf.push_back(energy);   
	OptFlowEnergy_Window(Energy_Frame);
	//Ee Sin 26 Nov
	Energy_Frame.release();
}
*/
//<<--
...
void ActivityDetection::CheckCurrentFrame()
{
	...
	string ImgStr;
	int	 PreBufferSize = 0;
	detScoreOut = detectScoreFrame * 0.2  + detectScoreWindow * 0.8;
