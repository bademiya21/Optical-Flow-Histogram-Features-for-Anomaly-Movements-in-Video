...
//extern "C" double *optimize_qp(QP *, double *, long, double *, LEARN_PARM *);
//extern "C" void   free_optimize_qp();
...
void svm_learn_classification(DOC **docs, double *class1, long int
	...
	/* class1:       Training labels (y-part, zero if test example for
	transduction) */
...
#if 0
DOC* null_doc(FNUM n_words)
{
	static DOC* doc = 0;
	if (!doc) {
		FVAL *nullword = (FVAL*)malloc(sizeof(FVAL)*n_words);
		FNUM i;
		for (i=0; i<n_words; i++) nullword[i] = 0;
		doc=create_example(-2,0,0,0.0,create_svector(nullword,n_words,"",1.0)); 
		free(nullword);
	}
	return doc;
}
...
#if 1
	long n_words = (model->sv_num>1)? model->supvec[1]->fvec->n_words : 0;
	WRD nullword;          /* origin of the space. */

	nullword.wnum=0;
	nulldoc=create_example(-2,0,0,0.0,create_svector(&nullword,n_words,"",1.0)); 
#else
	nulldoc=null_doc((model->sv_num>1)? model->supvec[1]->fvec->n_words : 0);
#endif
...
double estimate_r_delta(DOC **docs, long int totdoc, KERNEL_PARM *kernel_parm)
{
	...
	DOC *nulldoc;           /* assumes that the center of the ball is at the */
#if 1
	long n_words = (totdoc>0)? docs[0]->fvec->n_words : 0;
	WRD nullword;          /* origin of the space. */

	nullword.wnum=0;
	nulldoc=create_example(-2,0,0,0.0,create_svector(&nullword,n_words,"",1.0)); 
#else
	nulldoc=null_doc((totdoc>0)? docs[0]->fvec->n_words : 0);
#endif
